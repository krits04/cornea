...
f1<-read.fasta(file="test_b.fasta", seqtype= "AA", as.string= T, strip.desc= T)
f2<-read.fasta(file="test_b.fasta", seqtype= "AA", as.string= T, strip.desc= T)
attr<-getAnnot(f1)
out<-list()
n<-length(f1)
for(i in 1:n) out[[i]]<-c(f1[[i]], f2[[i]])
write.fasta(sequences= out, names= attr, nbchar= 200, file.out= "concat_c.fasta")
